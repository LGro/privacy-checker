...
public abstract class SortableAppListActivity extends FragmentActivity {
	 ...
	 *            the order direction
	 */
	protected void updateListView(ActionBar.Tab tab, AppsListOrder order,
			boolean ascending) {
		...
		getSupportFragmentManager().beginTransaction()
				.replace(getTargetContainer(), appsList).commit();

		UserStudyLogger.getInstance().log("appsList_updateOrder-" + tab.getText() + "_" + ascending);
	}
